...
class RawFileHandler(FileSystemEventHandler):
    ...
    def _handle(self, event):
        ...
        if not wait_until_stable(path, CONFIG.stable_check_delay_s):
            ...
            return
        if db.enqueue(self.conn, path):
            log.info("Enqueued %s", path.name)


def scan_existing(conn: sqlite3.Connection) -> int:
    """Pick up files already sitting in the inbox at startup."""
    added = 0
    for path in sorted(CONFIG.inbox.iterdir()):
        if path.is_file() and is_raw(path) and db.enqueue(conn, path):
...
def start_observer(conn: sqlite3.Connection) -> PollingObserver:
    ...
    observer.schedule(RawFileHandler(conn), str(CONFIG.inbox), recursive=False)
    observer.start()
